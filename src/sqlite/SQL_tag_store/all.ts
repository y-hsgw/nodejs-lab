...
// 2. all でパラメータ付き SELECT を実行
//    ${minId} の部分がバインドされ、同じ「SQL 文字列パターン」はキャッシュされる
const minId = 1;
const t1 = performance.now();
const rows1 = sql.all`SELECT * FROM users WHERE id >= ${minId} ORDER BY id`;
...
const rows2 = sql.all`SELECT * FROM users WHERE id >= ${minId} ORDER BY id`;
