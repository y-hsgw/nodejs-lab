...
try {
  ...
  let stats = await stat(symlinkPath, { bigint: false });
  ...
  stats = await lstat(symlinkPath, { bigint: false });
  console.log('After lutimes - atime:', stats.atime, 'mtime:', stats.mtime);

  // 6. シンボリックリンク参照先のタイムスタンプを確認
  stats = await stat(symlinkPath, { bigint: false });
  console.log('After lutimes - atime:', stats.atime, 'mtime:', stats.mtime);
} catch (error) {
